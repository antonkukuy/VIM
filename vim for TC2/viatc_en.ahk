...
SetGroupInfo()
{
	...
	GroupInfo_arr["z"] :="zz >>窗口分隔栏位于 50%`nzx >>窗口分隔栏位于 100%`nzi >>最大化左面板`nzo >>最大化右面板`nzt >>TC窗口保持最前`nzn >>最小化 Total Commander`nzm >>最大化 Total Commander`nzr >>恢复正常大小`nzv >>纵向/横向排列`nzs >>TC透明`nzf >>全屏TC`nzl >>最简TC`nzq >>退出TC`nza >>重启TC"
